...
class SecretsHelper {
		...
		if ( ! self::has_valid_encryption_key( $encryption_type ) ) {
			...
			if ( @file_exists( $path_to_wp_config ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
				...
				if ( ! $exists ) {
					...
					$handle = fopen( $path_to_wp_config, 'w' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_fopen, WordPress.WP.AlternativeFunctions.file_system_operations_fopen

					if ( $handle ) {
						foreach ( $config_file as $line ) {
							fwrite( $handle, $line ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_fwrite, WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
						}
	...
	public static function has_errors() {
		return 'yes' === get_option( 'ts_easy_integration_has_encryption_error', 'no' );
	}
}
